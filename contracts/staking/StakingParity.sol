...
contract StakingParity is IERC721Receiver, AccessControlEnumerable, Pausable {
    ...
    address tokenParityStorage;
    address parityToken ;
    address managementParity;
    address token1;
    ...
    constructor(
        ...
        require(_tokenParityStorage != address(0), "Formation.Fi: zero address");
    ...
    function rewardPerToken(uint256 _id) public view returns (uint256 _rewardPerTokenStored) {
        require(_id < packs.length, "Formation.Fi: no pack");
    ...
    function stake(
        ...
        require(amount_ != 0 || tokenId_ != 0, "Formation.Fi: ParityData.Amount is zero");
        _updateReward(tokenId_);
        if (amount_ != 0) {
            balances[to_] += amount_;
            totalSupply += amount_;
            IERC20(token1).safeTransferFrom(msg.sender, address(this), amount_);
        }
        if (tokenId_ != 0) {
            holders[tokenId_] = to_;
            IERC721(parityToken).safeTransferFrom(msg.sender, address(this), tokenId_);
        }
        ...
        if (amount_ != 0)
         {
            ...
            IERC20(token1).safeTransfer(to_, amount_);
        }
        if (tokenId_ != 0) {
    ...
    function exit(uint256 tokenId_, address _to) external {
        claim(tokenId_, _to);
        unstake(tokenId_, balances[msg.sender], _to);
    ...
    function boostingRewardFactor(
        ...
        require(_id < packs.length, "Formation.Fi: no pack");
        Pack memory _pack = packs[_id];
        address _user = holders[tokenId_];
        ParityData.Amount memory _amount0 = ITokenParityStorage(tokenParityStorage).tokenBalancePerToken(tokenId_);
        uint256 _amount1 = balances[_user];
        ParityData.Amount memory _amount;
        ParityData.Amount memory _value;
        ParityData.Amount memory _weights = ITokenParityStorage(tokenParityStorage).weightsPerToken(tokenId_);
        _amount.alpha = (_amount1 * _pack.weights.alpha) /ParityData.COEFF_SCALE_DECIMALS;
        _amount.beta = (_amount1 * _pack.weights.beta) /ParityData.COEFF_SCALE_DECIMALS;
        _amount.gamma = (_amount1 * _pack.weights.gamma) /ParityData.COEFF_SCALE_DECIMALS;
        _value.alpha = calculateFactor(_amount0.alpha, _amount.alpha, _pack.idealAmount.alpha, _pack.idealRatio.alpha);
        _value.beta = calculateFactor(_amount0.beta, _amount.beta, _pack.idealAmount.beta, _pack.idealRatio.beta);
        _value.gamma = calculateFactor(_amount0.gamma, _amount.gamma, _pack.idealAmount.gamma, _pack.idealRatio.gamma);
        _factor =  (_weights.alpha *  _value.alpha + _weights.beta * _value.beta +  _weights.gamma * _value.gamma) / ParityData.COEFF_SCALE_DECIMALS;
    ...
    function calculateFactor(uint256 _amount0, uint256 _amount1, uint256 _idealAmount, uint256 _idealRatio) public pure returns (uint256 _factor){
        if ((_amount0 == 0) || (_amount1 == 0)){
            _factor = 0;
        } else {
            uint256 _ratio = (_amount0 * ParityData.COEFF_SCALE_DECIMALS) / _amount1;
            if ((_ratio <= _idealRatio) || ( _amount1 >= _idealAmount)) {
                _factor = ParityData.COEFF_SCALE_DECIMALS;
            } else {
                _factor = (_amount1 * ParityData.COEFF_SCALE_DECIMALS) / _idealAmount;
            }    
        }
    }
    ...
    function getWeightedAmount(
       ...
       Pack memory _pack = packs[_id];
        ParityData.Amount memory _amount = ITokenParityStorage(tokenParityStorage).tokenBalancePerToken(_tokenId);
        _weightedAmount =
            (_amount.alpha *
             _pack.weights.alpha +
              _amount.beta *
             _pack.weights.beta +
            _amount.gamma *
             _pack.weights.gamma)/ParityData.COEFF_SCALE_DECIMALS;
    }

    function getTotalWeightedAmount( uint256 _id) public view returns (uint256 _weightedAmount) {
        (
            IERC20 _tokenAlpha,
            IERC20 _tokenBeta,
            IERC20 _tokenGamma
