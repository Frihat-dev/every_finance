...
library ParityLogic {
    ...
    function searchIndexEvent(
        ParityData.Event[] memory _data,
        uint256 _indexEvent
    ...
    function getTotalValueUntilLastEventPerProduct(
        ParityData.Event[] memory _data,
        uint256 _indexEvent,
        uint256 _id
    ...
    function getTotalValueUntilLastEvent(
        ParityData.Event[] memory _data,
        uint256 _indexEvent
    ) internal pure returns (ParityData.Amount memory _totalValue) {
        if (_data.length > 0) {
            for (uint256 i = 0; i < _data.length; ++i) {
                if (_data[i].index < _indexEvent) {
    ...
    function getTotalTokenValue(
        ...
        _totalValue = getNetTotalTokenValue(
            _tokenBalancePerToken,
            _depositBalancePerToken,
            _depositRebalancingBalancePerToken,
            _price
        );
        _totalValue +=
            (_withdrawalBalancePerToken.alpha *
                _price[0] +
                _withdrawalBalancePerToken.beta *
                _price[1] +
                _withdrawalBalancePerToken.gamma *
                _price[2]) /
            ParityData.FACTOR_PRICE_DECIMALS;
    }

    function getNetTotalTokenValue(
        ParityData.Amount memory _tokenBalancePerToken,
        ParityData.Amount memory _depositBalancePerToken,
        ParityData.Amount memory _depositRebalancingBalancePerToken,
        uint256[3] memory _price
        ...
        _value.alpha =
            (_tokenBalancePerToken.alpha * _price[0]) /
            ParityData.FACTOR_PRICE_DECIMALS +
            _depositBalancePerToken.alpha +
            _depositRebalancingBalancePerToken.alpha;
        _value.beta =
            (_tokenBalancePerToken.beta * _price[1]) /
            ParityData.FACTOR_PRICE_DECIMALS +
            _depositBalancePerToken.beta +
            _depositRebalancingBalancePerToken.beta;
        _value.gamma =
            (_tokenBalancePerToken.gamma * _price[2]) /
            ParityData.FACTOR_PRICE_DECIMALS +
            _depositBalancePerToken.gamma +
            _depositRebalancingBalancePerToken.gamma;
        return _value.alpha + _value.beta + _value.gamma;
    }

    function getAvailableTokenValue(
        ParityData.Event[] memory depositBalancePerTokenPerEvent,
        ...
        _indexDeposit = searchIndexEvent(
            depositBalancePerTokenPerEvent,
            _indexEvent
        );
        _indexRebalancingDeposit = searchIndexEvent(
            depositRebalancingBalancePerTokenPerEvent,
            _indexEvent
        );
        _value = ParityMath.mulMultiCoefDiv2(
            tokenBalancePerToken,
            _price,
            ParityData.FACTOR_PRICE_DECIMALS
        );
        if (_indexDeposit < MAX_INDEX_EVENT) {
            _value = ParityMath.add2(
                _value,
                depositBalancePerTokenPerEvent[_indexDeposit].amount
            );
        }
    ...
    function getTokenValueToRebalance(
        ParityData.Event[] memory depositBalancePerTokenPerEvent,
        ParityData.Event[] memory depositRebalancingBalancePerTokenPerEvent,
        ParityData.Amount memory tokenBalancePerToken,
        uint256 _indexEvent,
    ...
    function calculateWithdrawalData(
        uint256 _rate,
        uint256 _totalValue,
    ...
    {
        ParityData.Amount memory _weights = getWeightsFromToken(
            _tokenBalancePerToken,
            _price
        );
        ...
        _totalAmountToWithdrawFromDeposit = Math.min(
            _totalAmountToWithdraw,
            ParityData.COEFF_SCALE_DECIMALS * _depositValueTotal
        );
        if (_totalAmountToWithdrawFromDeposit > 0) {
            _amountToWithdrawFromDeposit.alpha = Math.mulDiv(
                _totalAmountToWithdrawFromDeposit,
                _depositValue.alpha,
                _depositValueTotal
            );
            _amountToWithdrawFromDeposit.beta = Math.mulDiv(
                _totalAmountToWithdrawFromDeposit,
                _depositValue.beta,
                _depositValueTotal
            );
            _amountToWithdrawFromDeposit.gamma = Math.min(
                _totalAmountToWithdrawFromDeposit -
        ...
        if (_totalAmountToWithdrawFromTokens > 0) {
            _amountToWithdrawFromTokens
                .alpha = (_totalAmountToWithdrawFromTokens * _weights.alpha);
            _amountToWithdrawFromTokens
                .beta = (_totalAmountToWithdrawFromTokens * _weights.beta);
            _amountToWithdrawFromTokens.gamma = Math.min(
                _totalAmountToWithdrawFromTokens *
                    ParityData.COEFF_SCALE_DECIMALS -
                    (_amountToWithdrawFromTokens.alpha +
                        _amountToWithdrawFromTokens.beta),
        ...
        )
    {
        ...
        _newValue = Math.mulDiv(
            _valueTotal,
            _weights.alpha,
            ParityData.COEFF_SCALE_DECIMALS
        );
        (
            _depositToRemove.alpha,
            _depositToAdd.alpha,
            _depositRebalancing.alpha,
            _withdrawalRebalancing.alpha
        ) = _calculateRebalancingData(
            _newValue,
            _oldValue.alpha,
            _price[0],
            _newDeposit,
            _depositBalance.alpha
        );
        ...
        (
            _depositToRemove.beta,
            _depositToAdd.beta,
            _depositRebalancing.beta,
            _withdrawalRebalancing.beta
        ) = _calculateRebalancingData(
            _newValue,
            _oldValue.beta,
            _price[1],
            _newDeposit,
            _depositBalance.beta
        );
        ...
        (
            _depositToRemove.gamma,
            _depositToAdd.gamma,
            _depositRebalancing.gamma,
            _withdrawalRebalancing.gamma
        ) = _calculateRebalancingData(
            _newValue,
            _oldValue.gamma,
            _price[2],
    ...
    function _calculateRebalancingData(
        ...
        pure
        returns (
            uint256 _depositToRemove,
            uint256 _depositToAdd,
            uint256 _depositRebalancing,
        ...
        )
    {
        ...
        if (_newValue < _oldValue) {
            ...
            _withdrawalRebalancing = Math.mulDiv(
                _deltaValue,
                ParityData.FACTOR_PRICE_DECIMALS,
                _price
            );
    ...
    function isCancelWithdrawalRequest(
        ParityData.Event[] memory _withdrawalBalancePerTokenPerEvent,
        uint256 _indexEvent
    ) internal pure returns (bool _isCancel, uint256 _index) {
        _index = searchIndexEvent(
            _withdrawalBalancePerTokenPerEvent,
            _indexEvent
        );
        if (_index < MAX_INDEX_EVENT) {
            if (
                (_withdrawalBalancePerTokenPerEvent[_index].amount.alpha +
    ...
    function getWeightsFromToken(
        ParityData.Amount memory _tokenBalancePerToken,
        uint256[3] memory _price
    ) internal pure returns (ParityData.Amount memory _weights) {
        uint256 _totalTokenValue = _tokenBalancePerToken.alpha *
            _price[0] +
            _tokenBalancePerToken.beta *
            _price[1] +
            _tokenBalancePerToken.gamma *
            _price[2];
    ...
    function getWithdrawalTokenFees(
        ParityData.Amount memory _fee,
        ParityData.Amount memory _amountToWithdrawFromTokens,
        uint256[3] memory _price
    ) internal pure returns (ParityData.Amount memory _withdrawalFees) {
        ...
        _withdrawalFees = ParityMath.mulDivMultiCoef2(
            _withdrawalFees,
            ParityData.FACTOR_PRICE_DECIMALS,
            _scaledPrice
        );
    ...
    function updateTokenFlowTime(
        uint256 _oldTokenTime,
        uint256 _oldTokens,
        uint256 _newTokens
    ) internal view returns (uint256 _newTokenTime) {
        _newTokenTime =
            (_oldTokens * _oldTokenTime + block.timestamp * _newTokens) /
            (_oldTokens + _newTokens);
    }
}
