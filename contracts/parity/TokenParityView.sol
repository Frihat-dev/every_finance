...
contract TokenParityView is  Ownable { 
    ...
    function getTotalDepositRebalancingUntilLastEvent(uint256 _tokenId, uint256 _indexEvent,  uint256 _id) public view returns 
        (uint256 _totalValue){
        ParityData.Event[] memory _depositRebalancingBalancePerTokenPerEvent = tokenParityStorage.getDepositRebalancingBalancePerTokenPerEvent(_tokenId);
        _totalValue = ParityLogic.getTotalValueUntilLastEventPerProduct(_depositRebalancingBalancePerTokenPerEvent, 
        _indexEvent, _id);     
    }
