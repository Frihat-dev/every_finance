...
describe("Investment", function () {
  ...
  describe("investmentParity ", function () {
    it("investmentParity  ", async function () {
      ...
      await investmentParity.connect(owner).rebalanceRequest([4,  ethers.parseUnits("1000","ether"), 3, 0, 0, [ethers.parseUnits("0.3","ether"), ethers.parseUnits("0.2","ether"), ethers.parseUnits("0.5","ether")]]);
      await investmentParity.connect(owner).withdrawRequest(4, ethers.parseUnits("0.5","ether"));
      await investmentParity.connect(owner).withdrawRequest(4, ethers.parseUnits("1","ether"));
      const tvl = await investmentParity.connect(owner).getTotalTokenValue(3);
      console.log(tvl);
      const tvlNet = await investmentParity.connect(owner).getTotalNetTokenValue(3);
      console.log(tvlNet);
      console.log(await investmentParity.connect(owner).getRebalancingFee(3));
      console.log(await investmentParity.connect(owner).getWithdrawalFee(3, ethers.parseUnits("0.5","ether")));
...
it("withdraw case  ", async function () {
  ...
  await investmentParity.connect(owner).withdrawRequest(3,  ethers.parseUnits("1","ether"));
  await investmentParity.connect(owner).withdrawRequest(4,  ethers.parseUnits("1","ether"));
